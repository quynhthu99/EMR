  ...
  <div class="two-factor-auth">
    <a-card>
      <a-row :gutter="16" class="p-4">
        <a-col :span="8">
          <div class="two-factor-auth--title">Đăng ký xác thực 2 yếu tố</div>
          ...
          <div class="two-factor-auth--content">
            <div class="two-factor-auth--step-list">
              <div class="two-factor-auth--step-item">
                <div class="two-factor-auth--step-title">Bước 1: Cài đặt ứng dụng xác thực</div>
                <div class="two-factor-auth--step-content">
                  <span
                    >Chúng tôi đề xuất các ứng dụng xác thực di động dựa trên đám mây như Authy, Duo
                    Mobile và LastPass. Họ có thể khôi phục quyền truy cập nếu bạn bị mất thiết bị
                    phần cứng.</span
                  >
                  <div class="mt-4">Tải app:</div>
                  <div class="two-factor-auth--download-app">
                    <div class="two-factor-auth--download-app-item">
                      <a
                      ...
                      >
                        <img
                          ...
                          class="two-factor-auth--download-app-item-img"
                          alt="appstore"
                        />
                      </a>
                    </div>
                    <div class="two-factor-auth--download-app-item">
                      <a href="https://play.google.com/store/search?q=authy&c=apps" target="_blank">
                        <img
              ...
              <div class="two-factor-auth--step-item">
                <div class="two-factor-auth--step-title">Bước 2: Quét QR code</div>
                <div class="two-factor-auth--step-content">
                  <ul>
                    <li>Vào app xác thực đã tải</li>
                    <li>Chọn chức năng quét QRCode</li>
                    <li>Quét mã QRCode sau</li>
                  </ul>
                  <div class="two-factor-auth--qrcode">
                    <img :src="urlImage" alt="qrCode" class="two-factor-auth--qrcode-img" />
                  </div>
                </div>
              </div>
              <div class="two-factor-auth--step-item">
                <div class="two-factor-auth--step-title">Bước 3: Nhập mã xác thực</div>
                <div class="two-factor-auth--step-content">
                  ...
                  <a-button type="primary" @click="confirmEnableTwoFactor">Xác thực</a-button>
                </div>
              </div>
            </div>
          </div>
...
import { requestEnableTwoFa, confirmEnableTwoFa } from '@/services/sys/staff/index.ts'
...
export default defineComponent({
  ...
  setup() {
    ...
    // METHOD
    const requestEnableTwoFactor = () => {
      requestEnableTwoFa().then((rs) => {
        if (rs && rs.body) {
          urlImage.value = rs.body.qrCode
        }
      })
    }
    ...
    const confirmEnableTwoFactor = () => {
      validate().then(() => {
        ...
        confirmEnableTwoFa(params).then((rs) => {
          if (rs && rs.body) {
            message.success({ content: 'Bật xác thực 2 yếu tố thành công' })
