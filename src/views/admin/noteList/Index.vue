<template>
            ...
            <va-table class="px-2" :columns-table="columns" :data-source="data" :spinning="loading"
                ...
                <template #bodyCell="{ column, record }">
                    <template v-if="column.key === 'action'">
                        <router-link v-if="record.type == 1" to="/hospital-checkin">
                            <a-button type="primary">
                                <template #icon>
                                    <FundViewOutlined />
                                </template>
                        ...
                        <router-link v-if="record.type == 2" to="/medical-record">
                            <a-button type="primary">
                                <template #icon>
                                    <FundViewOutlined />
                                </template>
                        ...
                        <router-link v-if="record.type == 3" to="/treatment-sheet">
                            <a-button type="primary">
                                <template #icon>
                                    <FundViewOutlined />
                                </template>
                    ...
                    <div class="">
                        <div class="flex justify-between py-2">
                            ...
                            <div class="lg:flex grid grid-cols-3 gap-2 max-w-[700px]">
                                ...
                                <a-button type="primary" :disabled="!isDisabled" @click="showDrawer">
                                    <template #icon>
                                        <plus-square-outlined />
                                    </template>
                                    Thêm mới
                                </a-button>
                                <a-popconfirm :disabled="!isDisabled" title="Bạn có thực sự muốn sửa câu hỏi này?"
                                    ok-text="Có" cancel-text="Không" @confirm="updateData" @cancel="cancel">
                                    <a-button :disabled="!isDisabled" @click="updateHandle(record)" type="primary">
                                        <template #icon>
                                            <tool-outlined />
                                        </template>
                                        Sửa
                        ...
                        <div class="">
                            <a-form v-if="isDisabled" :label-col="labelCol" :wrapper-col="wrapperCol">
                                ...
                                <a-form-item label="Phòng">
                                    <a-select disabled show-search placeholder="Chọn khoa" :options="options"
                                        :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur"
                                        @change="handleChange" v-model:value="record.room">
                                        <!-- {{record.department}} -->
                                    </a-select>
                                </a-form-item>
                            ...
                            <a-form v-else :label-col="labelCol" :wrapper-col="wrapperCol">
                                ...
                                <a-form-item label="Tên viết tắt">
                                    <a-input v-model:value="formUpdate.shortName" />
                                </a-form-item>
                                <a-form-item label="Mô tả">
                                    <a-textarea v-model:value="formUpdate.description" />
                                </a-form-item>
                                <a-form-item label="Khoa áp dụng ">
                                    <a-select show-search placeholder="Chọn khoa" :options="options"
                                        :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur"
                                        @change="handleChange" v-model:value="formUpdate.department">
                                        <!-- {{record.department}} -->
                                    </a-select>
                                </a-form-item>
                                ...
                                <a-form-item label="Đối tượng sử dụng">
                                    <a-select show-search placeholder="Chọn khoa" :options="options"
                                        :filter-option="filterOption" @focus="handleFocus" @blur="handleBlur"
                                        @change="handleChange" v-model:value="formUpdate.permissionUser">
                                        <!-- {{record.department}} -->
                                    </a-select>
                                </a-form-item>
                                <a-form-item label="Có hiệu lực">
                                ...
                                <div class="flex justify-end">
                                    <a-button class="mr-4" type="primary" @click="saveData">
                                        <template #icon>
                                            <SaveOutlined />
                                        </template>
                                        Lưu
                                    ...
                                    <!-- <div class="m"></div> -->
                                    <a-button type="primary" danger @click="saveData">
                                        <template #icon>
                                            <close-outlined />
                                        </template>
                                        Hủy
    ...
    <a-drawer title="Thêm mới" :width="720" :visible="visible" :body-style="{ paddingBottom: '80px' }"
        ...
        <a-form :model="form" :rules="rules" layout="vertical">
            <a-row :gutter="16">
                <a-col :span="12">
                    <a-form-item label="Tên phiếu" name="name">
                        <a-input v-model:value="form.name" placeholder="Please enter name" />
                    </a-form-item>
                </a-col>
                <a-col :span="12">
                    <a-form-item label="Tên viết tắt" name="shortName">
            ...
            <a-row :gutter="16">
                <a-col :span="12">
                    <a-form-item label="Khoa áp dụng" name="owner">
                ...
                <a-col :span="12">
                    <a-form-item label="Đối tượng sử dụng" name="type">
...
import { SearchOutlined, PlusSquareOutlined, SaveOutlined, ToolOutlined, DeleteOutlined, HistoryOutlined, CloseOutlined, FundViewOutlined } from '@ant-design/icons-vue';
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const rules: Record<string, Rule[]> = {
            ...
            department: [{ required: true, message: 'Please select department' }],
            permissionUser: [{ required: true, message: 'Please choose the type' }],
        ...
        const updateHandle = (record) => {
            formUpdate.name = record.name
            formUpdate.shortName = record.shortName
            formUpdate.department = record.department
            formUpdate.permissionUser = record.permissionUser
            formUpdate.description = record.description
        }
